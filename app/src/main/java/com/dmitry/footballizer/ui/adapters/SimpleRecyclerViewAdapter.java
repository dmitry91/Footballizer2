package com.dmitry.footballizer.ui.adapters;
...
public class SimpleRecyclerViewAdapter extends RecyclerView.Adapter<SimpleRecyclerViewAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        if (viewType == 0){
            view = mInflater.inflate(R.layout.simple_row_recyclerview, parent, false);
        }
        else {
            view = mInflater.inflate(R.layout.group_sip_row, parent, false);
        }
    ...
    public int getItemViewType(int position) {
        if(mData.get(position).indexOf("GROUP_") == -1) {
            return 1;
        }
        else{
            return 0;
